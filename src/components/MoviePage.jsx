...
function GenreElement({value}){
    return (
        <div className="w-auto h-7 border-1 rounded-xl text-center px-3 border-violet-500">
            <p>{value}</p>
        </div>
    )
}
    ...
    <div className="bg-black h-auto flex justify-around p-5 text-white">
        ...
        <div className='font-(--poppins)  md:h-full  md:p-5 md:pt-20 flex flex-col  gap-6'>
            ...
            <div onClick={hanldeFavClick} className='absolute right-5 top-2 w-10 h-10 cursor-pointer text-medium p-2 border-1 border-violet-500 rounded-full text-white'>
                ...
                <p className='text-5xl  font-medium'>{thisMovieData.primaryTitle} <span className='text-[#5d5b5b] text-xs '>{thisMovieData.releaseDate.substring(0,4)}</span> </p>
            ...
            <div className='description' >
                ...
                <div className='h-10 w-full flex gap-3 mt-2' >
                    {
                    ...
                    
            {
                ...
                <div className='trailer'>
                        <iframe width="914" height="514" src={link} title={thisMovieData.primaryTitle + "| Official Trailer"} frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerPolicy="strict-origin-when-cross-origin" allowFullScreen></iframe>
                </div>
            }
            <div className='' >
